...
namespace SourceDemoParser.Tests
{
    internal class Test
    {
        ...
        public async Task Net()
        {
            ...
            var parser = new SourceParser(ParsingMode.Header | ParsingMode.Packet);
            var demo = await parser.ParseFileAsync(Paths.Demos + source);
            ...
            foreach (var msg in demo.Messages)
            {
                if (msg.Frame is PacketFrame pf)
                {
                    lines.Add($"--- tick {msg.Tick} ---");
                    foreach (var nmsg in pf.NetMessages)
                    {
                        lines.Add($"{nmsg.Type.MessageType} {nmsg.Type.Name}");
                    }
                }
            }

            await System.IO.File.WriteAllLinesAsync("logs/dump.txt", lines);
        }
    }
}
