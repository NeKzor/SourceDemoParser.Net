...
namespace SourceDemoParser.Speedrun
{
    public static class Adjustments
    {
        ...
        public static async Task<SourceDemo> AdjustAsync(this SourceDemo demo)
        {
            ...
            {
                var packets = demo.MessagesWhereType(typeof(Packet));
                var cmds = demo.MessagesWhereType(typeof(ConsoleCmd));
                ...
                {
                    if (endtick != null)
                        return await demo.TimeExact(startTick: (int)starttick, endTick: (int)endtick)
                            .ConfigureAwait(false);
                    return await demo.TimeExact(startTick: (int)starttick).ConfigureAwait(false);
                }
                else if (endtick != null)
                    return await demo.TimeExact(endTick: (int)endtick).ConfigureAwait(false);
            }
            return demo;
        }
            ...
            Adjustment adjustment)
        {
            ...
            foreach (var message in messages)
            {
                ...
                if (adjustment.Parameter == PlayerStructType.Position)
                {
                    ...
                    {
                        Previous = cpos,
                        Current = (cpos = (message as Packet).Info[0].ViewOrigin)
                    };
                }
                else if (adjustment.Parameter == PlayerStructType.Command)
                {
                    ...
                    {
                        Previous = ccmd,
                        Current = (ccmd = (message as ConsoleCmd).Command)
                    };
                }
